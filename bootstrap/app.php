...
$builder->addDefinitions([
    ...
    \App\Middlewares\MaintainanceMode::class => \DI\autowire(
        \App\Middlewares\MaintainanceMode::class,
    ),
    \App\Middlewares\AuthMiddleware::class => \DI\autowire(
        \App\Middlewares\AuthMiddleware::class,
    ),
    \App\Middlewares\GuestMiddleware::class => \DI\autowire(
        \App\Middlewares\GuestMiddleware::class,
    ),
    \App\Middlewares\RolebasedMiddleware::class => \DI\autowire(
        \App\Middlewares\RolebasedMiddleware::class,
    ),
    \App\Middlewares\RemembermeMiddleware::class => \DI\autowire(
        \App\Middlewares\RemembermeMiddleware::class,
    ),
...
$container->set(Twig::class, function ($c) use ($app) {
    $twig = new Twig($c->get(LoaderInterface::class), [
        "cache" => false,
        "debug" => true,
    ]);
    ...
    $twig->getEnvironment()->addFilter(
        new TwigFilter("base64_encode", function ($string) {
...
if (!function_exists("storage_path")) {
    function storage_path(string $path = ""): string
    {
        ...
        return $path ? $storageDir . "/" . ltrim($path, "/") : $storageDir;
    }
}
...
if (!function_exists("storage_url")) {
    function storage_url(string $path = ""): string
    {
        ...
        return $path ? $storageUrl . "/" . ltrim($path, "/") : $storageUrl;
    }
}
...
// get base path
\App\Config\BasePath::$basePath = $_SERVER["HTTP_HOST"] ?? "localhost";
// Add Twig middleware
...
$customErrorHandler = function (
    ...
    if ($exception instanceof \Slim\Exception\HttpNotFoundException) {
        $response->getBody()->write($view->fetch("pages/404.twig"));
        return $response->withStatus(404);
    }

    if ($exception instanceof \Slim\Exception\HttpMethodNotAllowedException) {
        $response->getBody()->write($view->fetch("pages/405.twig"));
        return $response->withStatus(405);
    }

    // Fallback for all other errors
    $response->getBody()->write(
        $view->fetch("pages/500.twig", [
