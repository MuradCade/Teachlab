<?php
include('../model/dbcon.php');
if(isset($_GET['userid'])){
    $userid = base64_decode($_GET['userid']);

    $sql = "select * from forgetpwd where userid = '$userid'";
    $result = mysqli_query($connection,$sql);
    while($rows  = mysqli_fetch_assoc($result))
        if($rows['recoverstatus'] == '1'){
                        
            // echo $rows['recoverstatus'];
            header('location:login.php?reoverylinkexpired');
            exit();
           
        }else{
            if(isset($_POST['submit'])){
                $code = $_POST['code'];
                $pwd = $_POST['pwd'];
                   //hash the new password
                 $hashedpwd = password_hash($pwd, PASSWORD_DEFAULT);
                if(empty($code)){
                    header("location:recoverpassword.php?userid=".base64_encode($userid).'&emptycodefield');
                    exit();
                }else if(empty($pwd)){
                    header("location:recoverpassword.php?userid=".base64_encode($userid).'&emptypasswordfeild');
                    exit();
        
                }else{
                    $sql = "select * from forgetpwd where userid = '$userid'";
                    $result = mysqli_query($connection,$sql);
                    if($result){
                        while($row = mysqli_fetch_assoc($result)){
                            if($code == $row['code']){
                                $sql2 = "update forgetpwd set recoverstatus= '1'  where userid = '$userid'";
                                $result2 = mysqli_query($connection,$sql2);
                                if($result2){
                                    //update users table with new password
                                    $sql3 = "update users set pwd = '$hashedpwd' where userid = '$userid'";
                                    $result3 = mysqli_query($connection,$sql3);
                                    if($result3){
                                        header("location:login.php?pwdrecovered");
                                        exit();
                                    }else{
                                        header("location:recoverpassword.php?userid=".base64_encode($userid).'&query3failed');
                                         exit();
                                    }
                                }else{
                                    header("location:recoverpassword.php?userid=".base64_encode($userid).'&query2failed');
                                     exit();
                                }
                            }else{
                                header("location:recoverpassword.php?userid=".base64_encode($userid).'&wrongverificationcode');
                            exit();
                            }
                        }
                    }else{
                        header("location:recoverpassword.php?userid=".base64_encode($userid).'&query1failed');
                        exit();
                    }
                }
            }
        }

    

   
    
}else{
    header('location:login.php?redirect');
        exit();
}
?>

<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>TeachLab - Recover Password</title>

  <link rel="icon" type="image/x-icon" href="https://cdn.pixabay.com/photo/2012/04/24/12/46/letter-39873_640.png">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-icons/1.11.3/font/bootstrap-icons.min.css" integrity="sha512-dPXYcDub/aeb08c63jRq/k6GaKccl256JQy/AnOq7CAnEZ9FzSL9wSbcZkMp4R26vBsMLFYH4kQ67/bbV8XaCQ==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <script type="module" src="https://cdn.jsdelivr.net/gh/lekoala/pop-notify@master/pop-notify.min.js"></script>
    <link href="https://cdn.jsdelivr.net/gh/lekoala/pop-notify/pop-notify.css" rel="stylesheet">
  </head>
  <body style="background-color:#f2f2f2;">


      <div class="container">
      <div class="row">
      <div class="col-lg-8 col-md-8 col-sm-12 mx-auto">
        <div class="card mt-5 p-2">
        <div class="card-head mt-4 px-4 text-center mb-2">
         <div class="text-center">
             <img  class="mb-2 " width="150" src="data:image/png;base64,"/>
         </div>
          <h4 style="font-size:24px;" class="fw-bold mb-3">Recover Password</h4>
          
        </div>
       <?php if(isset($_GET['emptycodefield'])){?>
        <p class='bg-danger text-white fw-bold p-1 text-center ' style='font-size:15px;'>Empty Verification Code Field</p>
        <?php }?>
       <?php if(isset($_GET['emptypasswordfeild'])){?>
        <p class='bg-danger text-white fw-bold p-1 text-center ' style='font-size:15px;'>Empty Password Field</p>
        <?php }?>
       <?php if(isset($_GET['linkexpire'])){?>
        <p class='bg-danger text-white fw-bold p-1 text-center ' style='font-size:15px;'>Sorry , Link Expired.</p>
        <?php }?>
       <?php if(isset($_GET['wrongverificationcode'])){?>
        <p class='bg-danger text-white fw-bold p-1 text-center ' style='font-size:15px;'>Wrong Verification Code</p>
        <?php }?>
       <?php if(isset($_GET['query1failed']) ||isset($_GET['query2failed']) || isset($_GET['query3failed']) ){?>
        <p class='bg-danger text-white fw-bold p-1 text-center ' style='font-size:15px;'>Error, Please Try Again Later</p>
        <?php }?>
       
        
        <form method="post">
          <div class="form-group p-2">
              <label class="form-label" style="font-size:15px;">Verification Code</label>
              <input type="text" id="email" name="code" class="form-control" placeholder="Enter Verification Code..." autocomplete="false"/>
          </div>
          <div class="form-group p-2">
              <label class="form-label" style="font-size:15px;">New Password</label>
              <input type="text" id="email" name="pwd" class="form-control" placeholder="Enter New Password..." autocomplete="false"/>
          </div>
                <div class="px-2 mb-2">
                <input type="submit" class="mt-2 btn btn-primary" value="Submit" name='submit'/>
                </div>
          </form>
        </div>
      </div>
    </div>
      </div>


<script>        
let hide = document.getElementById('hide');
let show = document.getElementById('show');
let pwd = document.getElementById('pwd');



hide.addEventListener('click',function(){
        pwd.type = 'text';
        show.style.display='block';
        hide.style.display='none';

        
});
show.addEventListener('click',function(){
    pwd.type = 'password';
    hide.style.display='block';
    show.style.display='none';
});
    </script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>

  </body>
</html>

