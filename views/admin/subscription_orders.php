...
if(isset($_POST['update'])){
    ...
    $substatus = mysqli_escape_string($connection,$_POST['substatus']);
    $mainid = mysqli_escape_string($connection,$_POST['mainid']);
    if(empty($userid)){
        header('location:subscription_orders.php?details='.base64_encode($mainid).'&emptyuserid');
        exit();
    }else if(empty($substatus)){
        header('location:subscription_orders.php?details='.base64_encode($mainid).'&emptysubstatus');
        exit();
    }
    else{
       
            $sql = "update usersubscription_manager set subscription_status = '$substatus' where id = '$userid'";
            $result = mysqli_query($connection,$sql);
            if($result){
                
                    header('location:subscription_orders.php?details='.base64_encode($mainid).'&subscriptionupdatesuccess');
                    exit();
...
if(isset($_GET['delid'])){
    ...
    $mainid =$_GET['mainid'];
    $sql = "delete from usersubscription_manager where id = '$userid'";
    $result = mysqli_query($connection,$sql);
    if($result){
        header('location:subscription_orders.php?details='.$mainid.'&delsuccess');
                exit();
    }
}
...
<head>
    ...
    <title>TeachLab - Subscribtion Orders</title>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-10 col-md-10 col-xl-12 mb-4">
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <?php
                                $sql = "select * from usersubscription_manager group by userid  order by userid ";
                                $result = mysqli_query($connection,$sql);
                                if(mysqli_num_rows($result) == 0){
                                    ...
                                    <form method="post">
                                        <tr>
                                            ...
                                            <td><a href='subscription_orders.php?details=<?= base64_encode($row['userid'])?>' class='btn btn-secondary btn-sm fw-bold'>Details</a></td>
                                        </tr>
                                    </form>
                                    <?php $rowid++;}
                                }
