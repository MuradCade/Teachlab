...
class StudentController
{
    ...
    public function indexAddnewstudent(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
    ...
    public function storenewStudentData(ServerRequestInterface $request, ResponseInterface $response, array $args)
    {
        $userid = $this->session->get('userid');
        $toast = new Toast();
        $errors = [];
        ...
        try {
            ...
            if ($e->errorInfo[1] == 1062) {
                $this->session->set('success_msg', ['duplocated_studentid' => 'Student ID already exists. Please use a different Student ID.']);
                return Redirector::redirect_to('/teacher/student/addnewstudent');
            }
        }
    }
    ...
    public function fetchstudentAssociatedwithspecific_course(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        $errors = [];
        $toast = new Toast();
        $data = $request->getParsedBody();
        ...
        if (!$this->csrf->verify($csrf_token)) {
            ...
            $errors['coursename'] = 'Coursename is required to display associated students';
        }
    ...
    public function indexEditStudent(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        $studentid = base64_decode($args['studentid']);
        // dd($studentid);
        $toast = new Toast();
        $current_page = 'dashboard.student';
    ...
    public function UpdateStudent(ServerRequestInterface $request, ResponseInterface $response, array $args)
    {

        $errors = [];
        $toast = new Toast();
        $studentid_url = base64_decode($args['studentid']);
        $data = $request->getParsedBody();
        $csrf_token = trim($data['csrf_token']);
        $studentid = trim($data['studentid']);
        ...
        $this->session->set('success_msg', ['success' => 'Student Information Updated Successfully.']);
        return Redirector::redirect_to('/teacher/student/edit/' . base64_encode($studentid));
    }
    public function DeleteStudent(ServerRequestInterface $request, ResponseInterface $response, array $args)
    {
        $studentid = base64_decode($args['studentid']);
        Student::where('id', $studentid)
            ->where('userid', $this->session->get('userid'))
            ->delete();
        $this->session->set('success_msg', ['success' => 'Student Information Deleted Successfully.']);
